...
// === ANA033_DEBUG: Super-visible Debug HUD (fixed / iPad-safe) ===
(function(){
  try {
...
(function(){
  try{
    const BUILD_MARK = "V10FIX6-A-ANA033_DEBUG";


// === ANA033_DEBUG helpers ===
...
function ensureDbgPanel() {
  ...
  document.getElementById("dbgPanelClear")?.addEventListener("click", (e)=>{
    ...
    __DBG.buffer = [];
    dbg("ANA033_DEBUG cleared");
  }, {passive:false});
  dbg("ANA033_DEBUG panel ready");
}
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  dbg("ANA033_DEBUG observer active");
...
/* ===============================
   ANA030_DEBUG – Analytics Fix + Debug
   =============================== */
(() => {
  'use strict';
  const BUILD = 'ANA030_DEBUG';
...
// === ANA033_DEBUG: bind anaApply (Haupt-Aktualisieren) ===
(function(){
  try {
    const btn = document.getElementById('anaApply');
    ...
    btn.addEventListener('click', async ()=>{
      try {
        ...
        alert('ANA033_DEBUG: Fehler beim Aktualisieren: ' + (err && err.message ? err.message : String(err)));
      }
    });
...
// === ANA033_DEBUG: wrap createStay to surface errors ===
(function(){
  try {
    ...
    const wrapped = async function(...args){
      ...
      try {
        ...
        alert('ANA033_DEBUG: createStay Fehler: ' + (err && err.message ? err.message : String(err)));
        throw err;
      }
    };
