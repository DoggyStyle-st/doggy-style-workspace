...
// === ANA032_DEBUG: Super-visible Debug HUD (fixed / iPad-safe) ===
(function(){
  try {
...
(function(){
  try{
    const BUILD_MARK = "V10FIX6-A-ANA032_DEBUG";
...
// === ANA032_DEBUG: bind anaApply (Haupt-Aktualisieren) ===
(function(){
  try {
    const btn = document.getElementById('anaApply');
    ...
    btn.addEventListener('click', async ()=>{
      try {
        ...
        alert('ANA032_DEBUG: Fehler beim Aktualisieren: ' + (err && err.message ? err.message : String(err)));
      }
    });
...
// === ANA032_DEBUG: wrap createStay to surface errors ===
(function(){
  try {
    ...
    const wrapped = async function(...args){
      ...
      try {
        ...
        alert('ANA032_DEBUG: createStay Fehler: ' + (err && err.message ? err.message : String(err)));
        throw err;
      }
    };
