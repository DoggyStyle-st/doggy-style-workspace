...
const APP_BUILD = "ANA037P3_FIREBASE_INIT_FIX";
...
// Auth state
  CLOUD.auth.onAuthStateChanged(async (user)=>{
    ...
    if(!__authReadyResolved){
      ...
      __authReadyUser = user || null;
      try{ __authReadyResolve(__authReadyUser); }catch(_e){}
    }
    ...
    if(!user){
      ...
      // ANA036.2: when not logged in, go to login page and stop further init
      try{ location.href = "login.html"; }catch(_e){}
      return;
...
// Auth-Ready abwarten: erst wenn Firebase den Session-Status sicher aufgeloest hat,
  // starten wir Boot + Remote-Listener (verhindert iPad/Safari White-Screen / Race Conditions).
  await __authReadyPromise;
  if(!__authReadyUser){
    // Nicht eingeloggt -> Login-Seite (kein weiterer Init)
    try{ location.href = "login.html"; }catch(_e){}
    return;
  }

  // Jetzt erst: Boot + Listener (nur einmal)
  if(!__postAuthInitDone){
    __postAuthInitDone = true;
    await __postAuthInit();
  }
