...
const APP_BUILD = "V10FIX6-A-ANA-002";
...
function updateSyncUI(){
  ...
  // Internet-Status in iOS/PWA ist unzuverlässig. Wir behandeln 'Cloud OK' als Online-Indikator.
  const netOnlineRaw = (typeof navigator !== 'undefined') ? !!navigator.onLine : false;
  const cloudSeemsOk = !!(cloudIsEnabled() && CLOUD.enabled && (SYNC.cloudLastOkAt || SYNC.cloudLastSeenAt));
  const netOnline = netOnlineRaw || cloudSeemsOk;
...
async function cloudLoadStateWithRetry(maxTries=3){
  let lastErr = null;
  // Beim Start des Reads sofort "Sync…" anzeigen, damit Online/Cloud sichtbar wird,
  // auch ohne dass der Nutzer zuerst speichert.
  try{
  ...
  for(let i=0;i<maxTries;i++){
    try{
      ...
      if(remote){
        ...
        return {remote, err:null};
      }
    }catch(e){
      lastErr = e;
      try{
        SYNC.cloudLastError = String(e && (e.message||e.error) || e);
        updateSyncUI();
      }catch(_){}
    }
