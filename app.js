...
function renderContractPanel(){
  ...
  // iOS/Safari: prevent scroll/zoom gestures on the canvas while drawing
  try{
  ...
  // NOTE: In app.html the button id is "contractSigBtn" (legacy). Keep compatibility.
  const _signBtn = document.getElementById("contractSigBtn") || document.getElementById("contractSignBtn");
  if(_signBtn) _signBtn.onclick = ()=>{
    const customerId = cs.value;
    const petId = ps.value;
    if(!customerId || !petId){ alert("Bitte Kunde und Hund auswählen."); return; }
...
function openContractFromStay(doc){
  ...
  // Signed-info is refreshed via the onchange handlers triggered above.
