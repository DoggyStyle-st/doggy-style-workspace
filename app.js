...
try{ window.CLOUD = CLOUD; }catch(_){ }
const ROLES = {
  ADMIN: 'admin',
  STAFF: 'staff',
  CUSTOMER: 'customer',
  ...
  try{
    ...
    try{
      if(!window.__dsCustomerDogsClickLock){
        window.__dsCustomerDogsClickLock = true;
...
async function initCustomerPortal(){
  ...
  async function openCustomerTask(task){
    ...
    const titleEl = document.getElementById('editorTitle') || document.getElementById('customerTaskTitle');
    const metaEl = document.getElementById('editorMeta') || document.getElementById('customerTaskMeta');
    ...
    if(nameEl) nameEl.value = currentDoc.title || '';
    if(dogSel){
      dogSel.innerHTML = currentDoc.dogId ? `<option value="${escapeHtml(currentDoc.dogId)}">${escapeHtml(currentDoc.dogId)}</option>` : '';
      dogSel.value = currentDoc.dogId || '';
    ...
    const saveDraftNow = async ()=>{
      try{
        ...
        currentDoc.fields = fields; currentDoc.meta = meta;
...
async function initStaffFeatures(){
  ...
  // Inbox: nicht blockierend initialisieren, damit Eingänge/Diagnose auch offline starten
...
async function wireTaskCreation(){
  ...
  // Der aktive, saubere Aufbau läuft jetzt ausschließlich über den Bereich
  // „Eingänge -> Aufgabe freigeben“ (wireInboxAssignments).
  try{
...
async function fetchInboxAssignmentCloudCustomers(){
  ...
  const add = (raw, source='cloud-users')=>{
    ...
    const portalUid = String(raw.uid || raw.portalUid || raw.userUid || raw.id || '').trim();
    const email = String(raw.email || raw.mail || '').trim().toLowerCase();
    const name = String(raw.displayName || raw.name || raw.fullName || '').trim();
    const phone = String(raw.phone || raw.tel || raw.mobile || '').trim();
    const key = String(portalUid || email || name || phone).toLowerCase();
    if(!key || seen.has(key)) return;
    seen.add(key);
    out.push({
      id: portalUid || email || name,
      customerId: portalUid || email || name,
      portalUid,
      name: name || email || phone || 'Kunde',
      email,
      phone,
...
function getMergedInboxAssignmentCustomers(){
  ...
  const add = (c)=>{
    if(!c) return;
    const portalUid = String(c.portalUid || c.uid || '').trim();
  ...
  try{ (Array.isArray(__inboxAssignmentCloudCustomers) ? __inboxAssignmentCloudCustomers : []).forEach(add); }catch(_){ }
  try{ getInboxAssignmentLocalCustomers().forEach(add); }catch(_){ }
  return merged.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''), 'de'));
}
...
async function wireInboxAssignments(){
  ...
  btnCreate.onclick = async ()=>{
    try{
      ...
      const tasksCol = cloudTasksCol();
      if(!tasksCol){
        setMsg('❌ Cloud nicht bereit. Aufgabe konnte nicht freigegeben werden.', true);
        try{ window.__dsInboxAssignDiag = { ...(window.__dsInboxAssignDiag||{}), createStatus: 'cloud-unavailable', createVia, createCustomer: task.customerName || task.customerEmail || task.customerUid || '-', createTask: task.taskId, error: 'cloud-unavailable' }; setDiag(); }catch(_){ }
        return;
      }
      if(!portalUid && !task.customerEmail){
        setMsg('Kunde sichtbar, aber weder Portal-Konto noch E-Mail-Zuordnung vorhanden.', true);
        try{ window.__dsInboxAssignDiag = { ...(window.__dsInboxAssignDiag||{}), createStatus: 'missing-target', createVia, error: 'missing-target' }; setDiag(); }catch(_){ }
        return;
      }
      ...
      saveState();
      selTemplate.value = '';
      setMsg(`✅ Aufgabe freigegeben für ${customer.name || customer.email || portalUid || 'Kunde'}.`, false);
      try{ window.__dsInboxCreateStatus = 'created'; window.__dsInboxAssignDiag = { ...(window.__dsInboxAssignDiag||{}), createStatus: 'created', createVia: portalUid ? createVia : (createVia + '+email'), createCustomer: task.customerName || task.customerEmail || task.customerUid || '-', createTask: task.taskId, error: '' }; setDiag(); }catch(_){ }
    }catch(e){
...
function renderAnalyticsPanel(){
  ...
  const stornoReasonRows = (() => {
    ...
    // Expose last analytics result for exports/debug
    try{
      window.__DS_ANA_LAST_PAID = paidInv;
      window.__DS_ANA_LAST_RANGE = { from: fromStr, to: toStr };
    }catch(__){}
    // Totals
    let gross = 0;
    let net = 0;
...
// --- 4D-2: Auslastung/Kapazität im gewählten Zeitraum ---
    ...
    if(btn && !btn.__dsBound){
      ...
      const setRange = (fromDate, toDate) => {
        try{
          const f = document.getElementById('anaFrom');
          const t = document.getElementById('anaTo');
          if(f) f.value = fromDate;
          if(t) t.value = toDate;
...
const EMBEDDED_CUSTOMER_DATA_TEMPLATE = {
  id: "customer_data",
...
const EMBEDDED_BOARDING_CONTRACT_TEMPLATE = {
  ...
  dsGvoNote: "Mit dem Absenden bestätige ich die Zustimmung zum Betreuungsvertrag.",
  embedded: true
};
function ensureEmbeddedTemplates(){
  try{
    ...
    const required = [
      ...
      EMBEDDED_CUSTOMER_DATA_TEMPLATE,
      EMBEDDED_BOARDING_CONTRACT_TEMPLATE
    ];
    required.forEach(t=>{
      if(!templates.some(x=>x && x.id === t.id)){
        templates.push(JSON.parse(JSON.stringify(t)));
      }
    });
  }catch(_){ /* ignore */ }
}
...
function cpRefreshVaccinationPhotoPreview(){
  ...
  if(!img || !btn) return;
  if(cpVaccinationPassDataUrl){ img.src = cpVaccinationPassDataUrl; img.style.display = "block"; btn.style.display = "inline-flex"; }
  else { img.removeAttribute('src'); img.style.display = "none"; btn.style.display = "none"; }
}
function cpRefreshProfilePhotoPreview(){
...
function cpWirePetPhotoInputs(){
  ...
  const profileInput = document.getElementById("p_profilePhoto");
  ...
  if(vaccBtn && !vaccBtn.dataset.wired){
    vaccBtn.dataset.wired='1';
    vaccBtn.addEventListener('click', ()=>{ cpVaccinationPassDataUrl=''; if(vaccInput) vaccInput.value=''; cpRefreshVaccinationPhotoPreview(); });
  }
  ...
  if(profileBtn && !profileBtn.dataset.wired){
    ...
    profileBtn.addEventListener('click', ()=>{ cpProfilePhotoDataUrl=''; if(profileInput) profileInput.value=''; cpRefreshProfilePhotoPreview(); });
  }
  cpRefreshVaccinationPhotoPreview();
  cpRefreshProfilePhotoPreview();
}
...
function openCpEditor(mode, petId){
  ...
  try{ cpWirePetPhotoInputs(); }catch(_){ }
...
function dsChooseBestCustomerForReview(list, petName, chipNumber){
  try{
    ...
    arr.forEach(function(c){
      ...
      if(cid){
        const own = pets.filter(x=>norm(x && (x.customerId || x.ownerId || '')) === cid);
        if(own.length === 1) score += 12;
        if(own.length > 0) score += Math.min(own.length, 5);
        if(petNameNorm){
          const sameName = own.filter(p=>lower(p && p.name) === petNameNorm);
          if(sameName.length === 1) score += 18;
          else if(sameName.length > 1) score += 8;
          sameName.forEach(function(p){
            score += dsScorePetForReview(p);
            if(chipNorm && norm(p && p.chipNumber) === chipNorm) score += 16;
          });
        }
        if(chipNorm && !petNameNorm){
          const sameChip = own.filter(p=>norm(p && p.chipNumber) === chipNorm);
          if(sameChip.length === 1) score += 18;
          else if(sameChip.length > 1) score += 8;
        }
      }
...
function dsOwnedPetsForCustomer(customerObj){
  try{
    const cid = norm(customerObj && (customerObj.id || customerObj.customerId || ''));
    if(!cid) return [];
    const pets = asArray(state && (((state.pets||[]).length ? state.pets : state.dogs) || []));
    return pets.filter(function(x){ return norm((x && (x.customerId || x.ownerId || '')) || '') === cid; });
  }catch(_){ return []; }
}
...
function dsFindExistingPetForInboxRow(customerObj, pet, row){
  try{
    ...
    let resolvedCustomer = customerObj || null;
    if(!resolvedCustomer) resolvedCustomer = dsFindExistingCustomerForInboxRow(row, (payload && payload.customer) || (row && row.customer) || {}) || null;
    const cid = norm((pet && pet.customerId) || (resolvedCustomer && (resolvedCustomer.id || resolvedCustomer.customerId)) || '');
    const chip = norm(pet && pet.chipNumber);
    const name = lower(pet && pet.name);
    let list = pets;
    if(cid) list = list.filter(x=>norm((x && (x.customerId || x.ownerId || '')) || '') === cid);
    if(list.length === 1) return list[0];
    if(chip){
      const hits = list.filter(x=>norm(x && x.chipNumber) === chip);
      if(hits.length === 1) return hits[0];
      if(hits.length > 1) return dsChooseBestPetForReview(hits);
    }
    if(name){
      const hits = list.filter(x=>lower(x && x.name) === name);
      if(hits.length === 1) return hits[0];
      if(hits.length > 1){
        if(chip){
          const chipHit = hits.filter(x=>norm(x && x.chipNumber) === chip);
    ...
    if(resolvedCustomer){
      const own = dsOwnedPetsForCustomer(resolvedCustomer);
      if(own.length === 1) return own[0];
      if(own.length > 1){
        if(name){
          const sameName = own.filter(x=>lower(x && x.name) === name);
          if(sameName.length === 1) return sameName[0];
          if(sameName.length > 1) return dsChooseBestPetForReview(sameName);
        }
        if(chip){
          const sameChip = own.filter(x=>norm(x && x.chipNumber) === chip);
          if(sameChip.length === 1) return sameChip[0];
          if(sameChip.length > 1) return dsChooseBestPetForReview(sameChip);
        }
      }
    }
  }catch(_){ }
...
function dsEnsureCanonicalProposalTargets(row, customerPayload, petPayload, currentCustomer, currentPet){
  try{
    ensureStateShape();
    let customer = currentCustomer || null;
    let pet = currentPet || null;
    const customers = asArray(state && state.customers);
    ...
    const pchip = norm((petPayload && petPayload.chipNumber) || (row && row.chipNumber) || '');
    ...
    if(!pet || !customer){
      ...
      if(pchip){
        const chipHits = hits.filter(function(d){ return norm(d && d.chipNumber) === pchip; });
      ...
      if(cphone){
        const phoneHits = hits.filter(function(d){ return norm((d && (d.phone || d.ownerPhone || d.customerPhone)) || '') === cphone; });
        if(phoneHits.length) hits = phoneHits;
      }
      if(hits.length === 1) legacy = hits[0] || null;
      else if(!hits.length && legacyDogs.length === 1) legacy = legacyDogs[0] || null;
    ...
    if(legacy){
      if(!customer){
        let existingCustomer = customers.find(function(c){
          return (cemail && lower(c && c.email) === cemail)
            || (cname && lower(c && c.name) === cname)
            || (cphone && norm(c && c.phone) === cphone);
        }) || null;
        if(existingCustomer) customer = existingCustomer;
      }
      if(!customer){
        const customerId = 'legacycust_' + norm((legacy && legacy.id) || '') || uid();
        customer = {
          id: customerId || uid(),
          customerId: customerId || uid(),
          name: String((customerPayload && customerPayload.name) || legacy.owner || legacy.ownerName || row.customerName || 'Kunde').trim() || 'Kunde',
          email: String((customerPayload && customerPayload.email) || legacy.email || legacy.ownerEmail || row.customerEmail || '').trim(),
          phone: String((customerPayload && customerPayload.phone) || legacy.phone || legacy.ownerPhone || row.customerPhone || '').trim(),
          street: String((customerPayload && customerPayload.street) || legacy.street || '').trim(),
          zip: String((customerPayload && customerPayload.zip) || legacy.zip || '').trim(),
          city: String((customerPayload && customerPayload.city) || legacy.city || '').trim(),
          note: String((customerPayload && customerPayload.note) || legacy.note || '').trim(),
          __reviewTemp: true
        };
        if(!customers.some(function(c){ return norm(c && (c.id || c.customerId)) === norm(customer.id || customer.customerId); })){
          state.customers.push(customer);
        }
      }
      if(!pet){
        const customerId = norm((customer && (customer.id || customer.customerId)) || '');
        let existingPet = pets.find(function(p){
          return (pchip && norm(p && p.chipNumber) === pchip)
            || ((pname && lower(p && p.name) === pname) && (!customerId || norm(p && (p.customerId || p.ownerId)) === customerId));
        }) || null;
        if(existingPet) pet = existingPet;
      }
      if(!pet){
        const customerId = norm((customer && (customer.id || customer.customerId)) || '');
        const petId = norm((legacy && legacy.id) || '') ? ('legacypet_' + norm(legacy.id)) : uid();
        pet = {
          id: petId || uid(),
          petId: petId || uid(),
          customerId: customerId,
          ownerId: customerId,
          name: String((petPayload && petPayload.name) || legacy.name || row.petName || 'Hund').trim() || 'Hund',
          breed: String((petPayload && petPayload.breed) || legacy.breed || '').trim(),
          birthdate: String((petPayload && (petPayload.birthdate || petPayload.birthDate)) || legacy.birthdate || legacy.birthDate || '').trim(),
          birthDate: String((petPayload && (petPayload.birthDate || petPayload.birthdate)) || legacy.birthDate || legacy.birthdate || '').trim(),
          sex: String((petPayload && petPayload.sex) || legacy.sex || '').trim(),
          chip: (petPayload && typeof petPayload.chip === 'boolean') ? petPayload.chip : !!legacy.chip,
          chipNumber: String((petPayload && petPayload.chipNumber) || legacy.chipNumber || '').trim(),
          vet: String((petPayload && petPayload.vet) || legacy.vet || '').trim(),
          vetPhone: String((petPayload && petPayload.vetPhone) || legacy.vetPhone || '').trim(),
          allergies: String((petPayload && petPayload.allergies) || legacy.allergies || '').trim(),
          medicalConditions: String((petPayload && petPayload.medicalConditions) || legacy.medicalConditions || '').trim(),
          feeding: String((petPayload && (petPayload.feeding || petPayload.food)) || legacy.feeding || legacy.food || '').trim(),
          food: String((petPayload && (petPayload.food || petPayload.feeding)) || legacy.food || legacy.feeding || '').trim(),
          behavior: String((petPayload && petPayload.behavior) || legacy.behavior || '').trim(),
          note: String((petPayload && petPayload.note) || legacy.note || '').trim(),
          profilePhoto: String(legacy.profilePhoto || '').trim(),
          vaccinationPassPhoto: String(legacy.vaccinationPassPhoto || '').trim(),
          __reviewTemp: true
        };
        if(!pets.some(function(p){ return norm((p && (p.id || p.petId)) || '') === norm(pet.id || pet.petId); })){
          state.pets.push(pet);
        }
        try{
          ...
          if(legacy && legacy.id){
            state._legacy.dogIdToPetId[legacy.id] = pet.id;
            state._legacy.dogIdToCustomerId[legacy.id] = customerId;
          }
        }catch(_){ }
      }
    }
...
function dsResolveProposalReviewTargets(sourceRow){
  try{
    ...
    if(customer && !pet){
      ...
      if(ownerHits.length === 1) pet = ownerHits[0] || null;
      else {
    ...
    if(!customer && pets.length){
      const pname2 = lower((petPayload && petPayload.name) || '');
      const pchip2 = norm((petPayload && petPayload.chipNumber) || '');
      let petHits = pets.slice();
      if(pchip2){
        const chipHits = petHits.filter(x=>norm(x && x.chipNumber) === pchip2);
        if(chipHits.length) petHits = chipHits;
      }
      if(pname2){
        const nameHits = petHits.filter(x=>lower(x && x.name) === pname2);
        if(nameHits.length) petHits = nameHits;
      }
      if(petHits.length === 1){
        pet = pet || petHits[0] || null;
    ...
    if(!customer || !pet){
      const bridged = dsEnsureCanonicalProposalTargets(row, customerPayload, petPayload, customer, pet);
      customer = customer || bridged.customer || null;
      pet = pet || bridged.pet || null;
    }
...
async function dsSaveProposalReview(){
  ...
  const chipSelect = String(document.getElementById('p_chipStatus')?.value || '');
  if(!customerName){ cpSetStatus('Bitte Kundennamen eintragen.', true); try{ alert('Bitte Kundennamen eintragen.'); }catch(_){ } return false; }
  ...
  try{
    ...
    const walk = function(root){
      try{
        ...
        nodes.forEach(function(el){ try{ if(el && el.shadowRoot) walk(el.shadowRoot); }catch(_){ } });
      }catch(_){ }
    };
    walk(document);
    return roots;
...
function dsScanDogsDom(){
  try{
    ...
    roots.forEach(function(root){
      try{
        const localItems = root.querySelectorAll ? Array.from(root.querySelectorAll('#dogList .item, section.panel#dogs .item, .panel#dogs .item, #dogs .item')) : [];
        localItems.forEach(function(el){ if(el && !seenItems.has(el)){ seenItems.add(el); items.push(el); } });
      }catch(_){ }
      try{
        const localRows = root.querySelectorAll ? Array.from(root.querySelectorAll('[data-role="pet-row"]')) : [];
        localRows.forEach(function(el){ if(el && !seenRows.has(el)){ seenRows.add(el); petRows.push(el); } });
      }catch(_){ }
      try{
        const localBtns = root.querySelectorAll ? Array.from(root.querySelectorAll('button, .smallbtn, [data-e="1"], [data-pet-edit-id]')) : [];
        localBtns.forEach(function(el){
          try{
            if(!el || seenBtns.has(el)) return;
            const txt = norm((el.textContent || el.value || '') || '');
            if(!/bearbeiten/i.test(txt)) return;
            seenBtns.add(el);
            buttons.push(el);
          }catch(_){ }
        });
    ...
    try{
      const rexSeen = new Set();
      items.forEach(function(el){
        try{
          if(!el || rexSeen.has(el)) return;
          rexSeen.add(el);
          const txt = lower((el.textContent || '') || '');
          if(/\brex\b/.test(txt)) rexCount += 1;
          if(/max mustermann/.test(txt)) customerCount += 1;
        }catch(_){ }
      });
    }catch(_){ }
    return { roots: roots.length || 0, items: items, petRows: petRows, buttons: buttons, rexCount: rexCount, customerCount: customerCount };
  }catch(_){ return { roots: 0, items: [], petRows: [], buttons: [], rexCount: 0, customerCount: 0 }; }
}

function dsCollectVisiblePetRowCandidates(){
  try{
    const scan = dsScanDogsDom();
    const items = (scan && Array.isArray(scan.items) && scan.items.length)
      ? scan.items
      : Array.from(document.querySelectorAll('#dogList .item, [data-tab-panel="dogs"] .item, #tab-dogs .item, section.panel#dogs .item, #dogs .item'));
    const seen = new Set();
    const out = [];
    const addItem = function(item, forcedBtn){
      try{
        if(!item || seen.has(item)) return;
        seen.add(item);
        const strong = item.querySelector ? item.querySelector('strong') : null;
        const strongTxt = norm((strong && strong.textContent) || '');
        const allBtns = item.querySelectorAll ? Array.from(item.querySelectorAll('button, .smallbtn, [data-e="1"], [data-pet-edit-id]')) : [];
        const btns = allBtns.filter(function(b){ return /bearbeiten/i.test(norm((b && b.textContent) || '')); });
        if(forcedBtn && !btns.includes(forcedBtn)) btns.unshift(forcedBtn);
        const itemTxt = norm((item.textContent || '') || '');
        const isPetLike = /^🐶/.test(strongTxt) || /bearbeiten/i.test(itemTxt) || /\brex\b/i.test(itemTxt);
        if(!isPetLike && !btns.length) return;
        let customerName = norm((item.dataset && item.dataset.customerName) || '');
        let customerId = norm((item.dataset && item.dataset.customerId) || (btns[0] && btns[0].dataset && btns[0].dataset.customerId) || '');
        if(!customerName || !customerId){
          let prev = item.previousElementSibling;
          while(prev){
            const ps = prev.querySelector ? prev.querySelector('strong') : null;
            const pst = norm((ps && ps.textContent) || '');
            if(pst && !/^🐶/.test(pst)){
              if(!customerName) customerName = pst.replace(/^[^\p{L}\p{N}]+/u,'').trim();
              if(!customerId) customerId = norm((prev.dataset && prev.dataset.customerId) || '');
              break;
            }
            prev = prev.previousElementSibling;
          }
        }
        const petName = (strongTxt || itemTxt).replace(/^🐶\s*/,'').trim();
        const btn = btns[0] || null;
        const petId = norm((item.dataset && item.dataset.petId) || (btn && btn.dataset && (btn.dataset.petEditId || btn.dataset.petId)) || '');
        out.push({ el:item, button:btn, petId:petId, customerId:customerId, customerName:customerName, petName:petName, hasDogIcon:/^🐶/.test(strongTxt) });
      }catch(_){ }
    };
    items.forEach(function(item){ addItem(item, null); });
    try{
      (scan.buttons || []).forEach(function(btn){
        try{
          const row = btn.closest ? (btn.closest('[data-role="pet-row"]') || btn.closest('.item')) : null;
          if(row) addItem(row, btn);
        }catch(_){ }
      });
    }catch(_){ }
    return out;
...
function dsFindVisiblePetRowForProposal(row){
  try{
    ...
    const list = document.getElementById('dogList');
    const globalScan = dsScanDogsDom();
    let petRows = [];
    try{ if(list) petRows = Array.from(list.querySelectorAll('[data-role="pet-row"]')); }catch(_){ petRows = []; }
    try{ if(!petRows.length && globalScan && Array.isArray(globalScan.petRows) && globalScan.petRows.length) petRows = globalScan.petRows.slice(); }catch(_){ }
    let usedRaw = false;
    let candidates = [];
    if(petRows.length){
      candidates = petRows.map(function(el){
        const btn = el ? el.querySelector('[data-pet-edit-id],[data-e="1"],button,.smallbtn') : null;
        return {
          el: el,
          button: btn || null,
          petId: norm((el.dataset && el.dataset.petId) || (btn && btn.dataset && (btn.dataset.petEditId || btn.dataset.petId)) || ''),
          customerId: norm((el.dataset && el.dataset.customerId) || (btn && btn.dataset && btn.dataset.customerId) || ''),
          customerName: norm((el.dataset && el.dataset.customerName) || (btn && btn.dataset && btn.dataset.customerName) || ''),
          petName: norm((el.dataset && el.dataset.petName) || ''),
          petChip: norm((el.dataset && el.dataset.petChip) || ''),
        };
      });
    } else {
      usedRaw = true;
      candidates = dsCollectVisiblePetRowCandidates().map(function(x){
        return {
          el: x.el,
          button: x.button,
          petId: norm(x.petId || ''),
          customerId: norm(x.customerId || ''),
          customerName: norm(x.customerName || ''),
          petName: norm(x.petName || ''),
          petChip: '',
        };
      });
    }
    if(!candidates.length){
      try{ if(row && typeof row === 'object'){ row.__openDiagRowCount='0'; row.__openDiagRaw=usedRaw?'1':'0'; row.__openDiagBtnCount='0'; row.__openDiagRootCount=String((globalScan && globalScan.roots) || 0); row.__openDiagGlobalBtnCount=String((globalScan && globalScan.buttons && globalScan.buttons.length) || 0); row.__openDiagGlobalRexCount=String((globalScan && globalScan.rexCount) || 0); row.__openDiagGlobalCustomerCount=String((globalScan && globalScan.customerCount) || 0); } }catch(_){ }
      return null;
    }
    let hits = candidates.slice();
    if(targetPid){ const x = hits.filter(function(c){ return norm(c.petId||'') === targetPid; }); if(x.length) hits = x; }
    if(targetCid){
      const x = hits.filter(function(c){ return norm(c.customerId||'') === targetCid; });
      if(x.length) hits = x;
      else {
    ...
    if(!hit && candidates.length === 1) hit = candidates[0] || null;
    const btnCount = candidates.filter(function(c){ return !!c.button; }).length;
    try{
      if(row && typeof row === 'object'){
        row.__openDiagRowCount = String(candidates.length || 0);
        row.__openDiagHitCount = String(hits.length || 0);
        row.__openDiagRowPetId = String((hit && hit.petId) || '--');
        row.__openDiagBtnPetId = String((hit && hit.button && hit.button.dataset && (hit.button.dataset.petEditId || hit.button.dataset.petId)) || '--');
        row.__openDiagRowCid = String((hit && hit.customerId) || '--');
        row.__openDiagBtnCount = String(btnCount || 0);
        row.__openDiagRaw = usedRaw ? '1':'0';
        row.__openDiagRootCount = String((globalScan && globalScan.roots) || 0);
        row.__openDiagGlobalBtnCount = String((globalScan && globalScan.buttons && globalScan.buttons.length) || 0);
        row.__openDiagGlobalRexCount = String((globalScan && globalScan.rexCount) || 0);
        row.__openDiagGlobalCustomerCount = String((globalScan && globalScan.customerCount) || 0);
      }
    }catch(_){ }
    if(!hit) return null;
    const petId = String((hit && hit.petId) || '').trim();
    return { row: hit.el || null, button: hit.button || null, petId: petId };
  }catch(_){ return null; }
}
...
function dsFindVisiblePetEditIdForRow(row){
  try{
    ...
    try{ if(list) petRows = Array.from(list.querySelectorAll('[data-role="pet-row"]')); }catch(_){ petRows = []; }
    try{ if(!petRows.length && globalScan && Array.isArray(globalScan.petRows) && globalScan.petRows.length) petRows = globalScan.petRows.slice(); }catch(_){ }
    try{ if(row && typeof row === 'object'){ row.__openDiagRootCount = String((globalScan && globalScan.roots) || 0); row.__openDiagGlobalBtnCount = String((globalScan && globalScan.buttons && globalScan.buttons.length) || 0); row.__openDiagGlobalRexCount = String((globalScan && globalScan.rexCount) || 0); row.__openDiagGlobalCustomerCount = String((globalScan && globalScan.customerCount) || 0); } }catch(_){ }
    if(!petRows.length){
      try{ const hit = dsFindVisiblePetRowForProposal(row); if(hit && hit.petId) return String(hit.petId || '').trim(); }catch(_){ }
      return '';
    }
...
function dsGetDirectProposalReviewPetId(row){
  try{
    ensureStateShape();
    const payload = row && (row.payloadSubmitted || row.payloadDraft || row.payload || row.data || {}) || {};
    const customerPayload = (payload && payload.customer && typeof payload.customer === 'object') ? payload.customer : (row && row.customer && typeof row.customer === 'object' ? row.customer : {});
    ...
    if(targetCid){
      const owned = pets.filter(function(x){ return norm((x && (x.customerId || x.ownerId || '')) || '') === targetCid; });
      if(owned.length === 1) return String((owned[0] && (owned[0].id || owned[0].petId)) || '').trim();
    }
    if(pets.length === 1) return String((pets[0] && (pets[0].id || pets[0].petId)) || '').trim();
    return '';
  }catch(_){ return ''; }
}
...
function dsOpenProposalInCustomerEditor(row, opts){
  ...
  try{
    ...
    if((!baseCustomer || !basePet || !basePetId) && basePetId){
      try{
        const allPets = asArray(state && (((state.pets||[]).length ? state.pets : state.dogs) || []));
        const hitPet = allPets.find(function(x){ return norm((x && (x.id || x.petId || '')) || '') === norm(basePetId); }) || null;
        if(hitPet) basePet = basePet || hitPet;
        if(!baseCustomer && hitPet && typeof getCustomer === 'function') baseCustomer = getCustomer(hitPet.customerId || hitPet.ownerId || '') || null;
      }catch(_){ }
    }
    if((!baseCustomer || !basePet || !basePetId)){
      try{
        const customers = asArray(state && state.customers);
        const pets = asArray(state && (((state.pets||[]).length ? state.pets : state.dogs) || []));
        if(!baseCustomer && customers.length === 1) baseCustomer = customers[0] || null;
        if(!basePet && pets.length === 1) basePet = pets[0] || null;
    ...
    if((!baseCustomer || !basePet || !basePetId)){
      ...
      if(uiPetId){
        ...
        try{ if(typeof openCpEditor === 'function') openCpEditor('edit', basePetId); }catch(_){ }
  ...
  const html = `
    ...
    <div class="pdf-page">
      <div class="page-inner">
        ...
        <h1>${isCancelled ? 'Stornorechnung' : 'Rechnung'}</h1>
        <p class="small">
...
function ensurePdfOverlayStyles(){
  ...
  body.printOverlayActive #pdfOverlay{
    ...
    .inv-doc > *{ position:relative; z-index:2; }
`;
...
$("#btnCpSave").addEventListener("click",(e)=>{
  ...
  try{
    if(__dsProposalReview && __dsProposalReview.active){
      dsSaveProposalReview();
      return;
    }
  ...
  saveBtn.onclick = async () => {
    ...
    try{
      ...
      try{
        ...
        alert('Statistik speichern fehlgeschlagen.' + detail);
      }catch(_){ }
    }finally{
...
function populateStatDogsForAll(dateISO){
  ...
  const sexEl = document.getElementById('statSex');
  ...
  const updateMeta = ()=>{
    ...
    if(!Number.isFinite(ageYears)){
      const bd = (p && (p.birthdate || p.birthday || p.dob || p.geburtsdatum)) ? String(p.birthdate||p.birthday||p.dob||p.geburtsdatum) : '';
      if(bd){
        const d = new Date(bd);
        if(!isNaN(d.getTime())){
          const now = new Date();
          ...
          if(years >= 0 && years <= 40) ageYears = years;
        }
      }
    }
    if(ageEl && Number.isFinite(ageYears)){
      if(!ageEl.value) ageEl.value = String(ageYears);
    }
  };
  ...
  try{
    ...
    rows.forEach(r=>{
      ...
      const line = [
        ...
        ctx.occupancy||'',
        ctx.trainer||'',
        ...STAT_DIMENSIONS.map(d=>{ const v=Number(r.__scores?.[d.id]); return Number.isFinite(v)? v : ''; }),
    ...
    const csv='\ufeff' + lines.join('\n');
    const stamp=(typeof toISODate==='function' ? toISODate(new Date()) : new Date().toISOString().slice(0,10)).replace(/-/g,'');
    ...
    try{
      ...
      a.click();
      exported = true;
      setTimeout(()=>{ try{ URL.revokeObjectURL(url); }catch(_){ } try{ a.remove(); }catch(_){ } }, 1200);
    }catch(_){ }
    ...
    if(msg) msg.textContent = exported ? `Export: ${rows.length} Zeilen.` : 'Export vorbereitet, Download bitte manuell speichern.';
    setTimeout(()=>{ if(msg) msg.textContent=''; }, 4000);
  }catch(e){
...
// Direct-Boot für Eingänge: unabhängig vom Staff-Init sofort starten
try{
  window.__dsInboxAssignDirectBoot = window.__dsInboxAssignDirectBoot || false;
  if(!window.__dsInboxAssignDirectBoot){
    window.__dsInboxAssignDirectBoot = true;
    const dsBootInboxAssign = ()=>{
      try{
        const sel = document.getElementById('assignCustomer');
        const tpl = document.getElementById('assignTemplate');
        const btn = document.getElementById('btnAssignTask');
        const diag = document.getElementById('assignDiag');
        if(!sel || !tpl || !btn) return;
        if(diag && /initialisiert|wird geladen/i.test(String(diag.textContent||''))){
          diag.textContent = 'Diagnose: Direct-Boot …';
        }
        Promise.resolve().then(()=>wireInboxAssignments()).catch(err=>{
          try{ if(diag) diag.textContent = 'Diagnose: Direct-Boot-Fehler · ' + String((err && err.message) || err || 'unbekannt'); }catch(_){ }
          console.warn('dsBootInboxAssign', err);
        });
      }catch(err){ console.warn('dsBootInboxAssign outer', err); }
    };
    if(document.readyState === 'loading'){
      document.addEventListener('DOMContentLoaded', ()=>setTimeout(dsBootInboxAssign, 50));
    }else{
      setTimeout(dsBootInboxAssign, 50);
    }
    window.addEventListener('load', ()=>setTimeout(dsBootInboxAssign, 50));
...
function dsChatTeamMembers(){
  return [
    { key:'all', label:'Alle', fullLabel:'Alle Chats', roleLabel:'Übersicht', email:'' },
...
function dsChatTeamByKey(key){
  ...
  return dsChatTeamMembers().find(x=>x.key===k) || dsChatTeamMembers().find(x=>x.key==='raphael');
}
...
function dsChatNormalizeTeamKey(value, allowAll){
  const v = String(value || '').trim().toLowerCase();
  if(allowAll && v === 'all') return 'all';
  return dsChatTeamMembers().some(x=>x.key===v && x.key !== 'all') ? v : 'raphael';
}
function dsChatFilterForTeam(chats, teamKey){
  const key = dsChatNormalizeTeamKey(teamKey, true);
  const arr = Array.isArray(chats) ? chats : [];
  if(key === 'all') return dsChatSortByUpdated(arr);
  return arr.filter(chat=> dsChatNormalizeTeamKey(chat?.teamMemberKey || chat?.channel || '') === key);
}
...
function dsChatErrorText(err){
  try{
    const code = String(err?.code || err?.name || '').trim();
    const msg = String(err?.message || err || '').trim();
    return [code, msg].filter(Boolean).join(' · ') || 'Unbekannter Fehler';
  }catch(_){ return 'Unbekannter Fehler'; }
}
...
async function dsSendChatMessage(chatId, text, extraMeta){
  ...
  const customerMeta = senderRole === 'staff'
    ...
    : {
        ...
        customerEmail: viewerProfile.customerEmail || '',
        customerName: viewerProfile.customerName || 'Kunde',
        customerId: viewerProfile.customerId || ''
      };
  ...
  await chatRef.set({
    ...
    customerName: customerMeta.customerName || 'Kunde',
    customerId: customerMeta.customerId || '',
    updatedAt: dsServerTimestamp(),
    updatedAtMs: now,
    lastMessageAt: dsServerTimestamp(),
    lastMessageAtMs: now,
...
function dsAdminChatState(){
  if(!window.__dsAdminChatState){
    window.__dsAdminChatState = { listUnsub:null, msgUnsub:null, chats:[], filteredChats:[], currentChatId:'', currentMessages:[], currentTeamKey:'all' };
  }
...
async function dsAdminEnsureChatForCustomer(customer, preferredTeamKey){
  ...
  try{
    if(customer.uid){
      ...
      byUid.forEach(push);
      debug.steps.push('uid=' + String(byUid.size || 0));
    }
    if(customer.email){
    ...
    if(customer.id){
      debug.steps.push('query:customerId');
      const byCustomerId = await col.where('customerId','==',customer.id).get();
      byCustomerId.forEach(push);
      debug.steps.push('customerId=' + String(byCustomerId.size || 0));
    }
  }catch(err){
    console.warn('admin ensure customer chat existing', err);
...
function renderAdminChatList(){
  ...
  if(openCustomerBtn && !openCustomerBtn.__chatBound){
    ...
    openCustomerBtn.onclick = ()=> dsAdminOpenChatForCustomer().catch(err=>{
      ...
      const hint = document.getElementById('chatAdminHint');
      if(hint) hint.textContent = '❌ ' + String(err?.message || err || 'Kundenchat konnte nicht geöffnet werden.');
    });
  }
...
async function dsOpenAdminChat(chatId){
  ...
  st.currentTeamKey = dsChatNormalizeTeamKey(current.teamMemberKey || st.currentTeamKey, true);
  renderAdminChatList();
  try{
...
async function dsLoadCustomerChatsForTeam(teamKey, autoCreate){
  ...
  if(!chats.length && autoCreate){
    const created = await ensureCurrentCustomerChat(key);
    chats = created ? [created] : [];
  }
  st.chats = chats;
  st.currentChatId = chats[0] ? String(chats[0].id || '') : '';
  renderCustomerChatMessages();
  if(st.currentChatId){
...
function renderCustomerChatPanel(forceReload){
  ...
  if(picker){
    picker.querySelectorAll('[data-customer-chat-team]').forEach(btn=>{
      ...
      btn.classList.toggle('primary', key === st.currentTeamKey);
      btn.onclick = async ()=>{
        if(dsChatNormalizeTeamKey(dsCustomerChatState().currentTeamKey) === key) return;
        const hint = document.getElementById('customerChatHint');
        try{
  ...
  }, (err)=>{ console.error('customer chat list', err); publish().catch(console.warn); });
...
// GB18: inline hard-click helpers for inbox buttons (Safari/iPad fallback)
try{
  window.__dsInboxRefreshButtonHard = async function(){
    try{
      const msgEl = document.getElementById('assignMsg');
      if(msgEl){ msgEl.textContent = 'Klick erkannt · Eingänge werden geladen …'; msgEl.style.color=''; }
    }catch(_){}
    if(window.__dsInboxForceRefresh) return await window.__dsInboxForceRefresh();
    if(window.wireInbox) return await window.wireInbox();
    return [];
  };
}catch(_){}

try{
  window.__dsInboxProposalButtonHard = async function(){
    try{
      const msgEl = document.getElementById('inboxProposalMsg');
      if(msgEl){ msgEl.textContent = 'Klick erkannt · Vorschläge werden geladen …'; msgEl.style.color=''; }
    }catch(_){}
    if(window.__dsInboxProposalDebugLoad) return await window.__dsInboxProposalDebugLoad();
    return [];
  };
}catch(_){}


// GB19: auto-load pending customer proposals directly into Eingänge selections and lists
try{
  window.__dsInboxAutoLoadProposals = async function(){
    const selCustomer = document.getElementById('assignCustomer');
    const selTemplate = document.getElementById('assignTemplate');
    const msgEl = document.getElementById('assignMsg');
    const diagEl = document.getElementById('assignDiag');
    const listEl = document.getElementById('inboxProposalList') || document.getElementById('inboxList');
    const setMsg = (txt, err=false)=>{ try{ if(msgEl){ msgEl.textContent = txt || ''; msgEl.style.color = err ? '#ffb3b3' : ''; } }catch(_){} };
    const rows=[]; const seen=new Set();
    const pushRow=(obj, source)=>{
      if(!obj || typeof obj !== 'object') return;
      const key = String(obj.id || obj.taskId || obj.proposalId || obj.submissionId || Math.random());
      const sig = source + ':' + key;
    ...
    try{
      const pcol = cloudProposalsCol();
      let snap;
      try{ snap = await pcol.where('proposalStatus','==','pending').limit(100).get(); }
      catch(_){ snap = await pcol.limit(100).get(); }
      snap && snap.forEach && snap.forEach(d => pushRow({ id:d.id, ...d.data() }, 'proposals'));
    }catch(err){ console.warn('GB19 cloud proposals load failed', err); }
    try{
      const tcol = cloudTasksCol();
      let snap;
      try{ snap = await tcol.where('status','==','submitted').limit(100).get(); }
      catch(_){ snap = await tcol.limit(100).get(); }
      snap && snap.forEach && snap.forEach(d => {
        const data = { id:d.id, ...d.data() };
        const pstatus = String((data && data.proposalStatus) || '').trim().toLowerCase();
        if(pstatus === 'pending' || String((data && data.status) || '').trim().toLowerCase() === 'submitted') pushRow(data, 'tasks');
      });
    }catch(err){ console.warn('GB19 cloud tasks load failed', err); }
    try{ (Array.isArray(state?.inboxAssignments)?state.inboxAssignments:[]).forEach(x=>pushRow(x,'state')); }catch(_){ }
    try{ (Array.isArray(state?.inboxSubmissions)?state.inboxSubmissions:[]).forEach(x=>pushRow(x,'state')); }catch(_){ }
    try{ (loadCustomerProposalBuffer() || []).forEach(x=>pushRow(x,'buffer')); }catch(_){ }
    rows.sort((a,b)=> Number((b&&b.submittedAt)||0) - Number((a&&a.submittedAt)||0));
    window.__dsInboxLoadedProposals = rows.slice();

    if(selCustomer){
      const prev=String(selCustomer.value||'');
      const opts=[]; const seenCust=new Set();
      rows.forEach(r=>{
        const cid = String(r.customerUid || r.portalUid || r.customerId || r.email || r.customerEmail || r.__key || '').trim();
        if(!cid || seenCust.has(cid)) return;
        seenCust.add(cid);
        const name = String(r.customerName || r.name || 'Kunde').trim();
        const mail = String(r.customerEmail || r.email || r.customerUid || '').trim();
        opts.push({value:cid, label: mail ? `${name} · ${mail}` : name});
      });
      if(opts.length){
        selCustomer.innerHTML = '<option value="">Bitte auswählen...</option>';
        opts.forEach(o=> selCustomer.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(o.value)}">${escapeHtml(o.label)}</option>`));
        if(prev && Array.from(selCustomer.options).some(o=>o.value===prev)) selCustomer.value=prev;
      }
    }
    if(selTemplate){
      const prev=String(selTemplate.value||'');
      const opts=[]; const seenTpl=new Set();
      rows.forEach(r=>{
        const tpl = String(r.templateId || r.proposalType || r.kind || r.formKey || 'customer_data').trim();
        if(!tpl || seenTpl.has(tpl)) return;
        seenTpl.add(tpl);
        let label = tpl;
        if(tpl === 'customer_data') label = 'Kundendaten ergänzen';
        else if(tpl === 'boarding_contract') label = 'Betreuungsvertrag';
        else if(tpl === 'stay_request') label = 'Neuer Aufenthalt';
        opts.push({value:tpl, label});
      });
      if(opts.length){
        selTemplate.innerHTML = '<option value="">Bitte auswählen...</option>';
        opts.forEach(o=> selTemplate.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(o.value)}">${escapeHtml(o.label)}</option>`));
        if(prev && Array.from(selTemplate.options).some(o=>o.value===prev)) selTemplate.value=prev;
      }
    }
    if(listEl){
      listEl.innerHTML='';
      if(!rows.length){
        ...
        rows.forEach(r=>{
          ...
          div.className='list-item';
          const title = r.templateId || r.proposalType || r.kind || r.formKey || 'Änderungsvorschlag';
          const name = r.customerName || r.name || r.customerEmail || r.email || r.customerUid || 'Kunde';
          const when = r.submittedAt ? fmtDT(r.submittedAt) : '';
          div.innerHTML = `<div><strong>${escapeHtml(String(title))}</strong><small>${escapeHtml(String(name))}${when?(' · '+when):''}</small></div>`;
          listEl.appendChild(div);
        });
      }
    }
    setMsg(rows.length ? ('Kundenvorschläge geladen: ' + rows.length) : 'Keine Kundenvorschläge gefunden.', !rows.length);
    try{ if(diagEl) diagEl.textContent = 'Diagnose: phase=gb19-autoload · proposals=' + rows.length; }catch(_){ }
    return rows;
  };

  window.__dsInboxMaybeAutoLoad = function(){
    try{
      const inboxCard = document.getElementById('taskAssignCard') || document.getElementById('inboxProposalDebug') || document.getElementById('inboxList');
      if(!inboxCard) return;
      clearTimeout(window.__dsInboxAutoLoadTimer);
      window.__dsInboxAutoLoadTimer = setTimeout(()=>{
        Promise.resolve().then(()=>window.__dsInboxAutoLoadProposals && window.__dsInboxAutoLoadProposals()).catch(err=>console.warn('GB19 autoload failed', err));
      }, 80);
    }catch(err){ console.warn('GB19 maybe autoload failed', err); }
  };

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ()=>setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120));
  else setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120);
  window.addEventListener('load', ()=>setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120));
  window.addEventListener('pageshow', ()=>setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120));
  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120); });
  document.querySelector('[data-tab="inbox"]')?.addEventListener('click', ()=>setTimeout(()=>window.__dsInboxMaybeAutoLoad && window.__dsInboxMaybeAutoLoad(), 120));
  ...
  document.addEventListener('click', function(ev){
    ...
    ev.preventDefault();
    ev.stopPropagation();
    Promise.resolve().then(()=>window.__dsInboxAutoLoadProposals && window.__dsInboxAutoLoadProposals()).catch(err=>console.warn('GB19 manual autoload failed', err));
  }, true);
}catch(err){ console.warn('GB19 bootstrap failed', err); }


// GB23: map pending customer proposals from Firestore directly into Eingänge selections
(function(){
  const wait = (ms)=>new Promise(r=>setTimeout(r, ms));
  const setText = (id, txt, isErr=false)=>{ try{ const el=document.getElementById(id); if(el){ el.textContent = txt || ''; el.style.color = isErr ? '#ffb3b3' : ''; } }catch(_){ } };
  const optExists = (sel, value)=>{ try{ return Array.from(sel.options||[]).some(o=>String(o.value||'')===String(value||'')); }catch(_){ return false; } };
  const addOption = (sel, value, label)=>{
    if(!sel || !String(value||'').trim()) return;
    try{
      if(optExists(sel, value)) return;
      const o = document.createElement('option');
      o.value = String(value||'').trim();
      o.textContent = String(label || value || '').trim();
      sel.appendChild(o);
    }catch(_){ }
  };
  const normTemplate = (row)=>{
    const raw = String(row.templateId || row.proposalType || row.kind || row.formKey || '').trim().toLowerCase();
    if(raw === 'customer_data' || raw === 'customer-main-dogs' || raw === 'dogs' || raw === 'kunde_hund' || raw === 'kundehund') return { id:'customer_data', name:'Kundendaten ergänzen' };
    if(raw === 'boarding_contract' || raw === 'contract' || raw === 'betreuungsvertrag') return { id:'boarding_contract', name:'Betreuungsvertrag freigeben' };
    if(raw === 'stay_request' || raw === 'stay' || raw === 'aufenthalt' || raw === 'new_stay') return { id:'stay_request', name:'Aufenthalt anfragen' };
    return { id:'customer_data', name:'Kundendaten ergänzen' };
  };
  const rowToCustomer = (row)=>{
    const payload = row.payloadSubmitted || row.payload || {};
    const c = payload.customer || row.customer || {};
    const email = String(row.customerEmail || c.email || row.email || '').trim().toLowerCase();
    const name = String(row.customerName || c.name || row.name || '').trim();
    const uid = String(row.customerUid || row.uid || row.portalUid || '').trim();
    const id = String(row.customerId || uid || email || name || row.id || '').trim();
    return {
      id: id || uid || email || name,
      customerId: id || uid || email || name,
      portalUid: uid,
      email,
      name: name || email || 'Kunde',
      phone: String(c.phone || row.phone || '').trim(),
      source: row.__source || 'proposal'
    };
  };
  const dedupe = (rows)=>{
    const seen = new Set(); const out=[];
    rows.forEach(r=>{
      const key = String(r.id || r.taskId || r.proposalId || '') || JSON.stringify([r.customerEmail,r.customerName,r.templateId,r.submittedAt]);
      if(!key || seen.has(key)) return; seen.add(key); out.push(r);
    });
    return out;
  };
  async function loadRows(){
    const rows=[];
    const push=(r,src)=>{ if(!r||typeof r!=='object') return; rows.push({__source:src, ...r}); };
    try{
      const pcol = typeof cloudProposalsCol==='function' ? cloudProposalsCol() : null;
      if(pcol && typeof pcol.where==='function'){
        let snap=null;
        try{ snap = await pcol.where('proposalStatus','==','pending').limit(100).get(); }catch(err){ console.warn('GB23 proposals query failed', err); }
        if(snap && typeof snap.forEach==='function') snap.forEach(d=>push({id:d.id, ...d.data()}, 'cloud-proposals'));
      }
    }catch(err){ console.warn('GB23 cloud proposals load failed', err); }
    try{
      const tcol = typeof cloudTasksCol==='function' ? cloudTasksCol() : null;
      if(tcol && typeof tcol.where==='function'){
        let snap=null;
        try{ snap = await tcol.where('proposalStatus','==','pending').limit(100).get(); }catch(err){ console.warn('GB23 tasks query failed', err); }
        if(snap && typeof snap.forEach==='function') snap.forEach(d=>push({id:d.id, taskId:d.id, ...d.data()}, 'cloud-tasks'));
      }
    }catch(err){ console.warn('GB23 cloud tasks load failed', err); }
    try{ (loadCustomerProposalBuffer()||[]).forEach(x=>push(x,'proposal-buffer')); }catch(_){ }
    try{ (Array.isArray(state?.inboxAssignments)?state.inboxAssignments:[]).forEach(x=>push(x,'state-assignments')); }catch(_){ }
    try{ (Array.isArray(state?.inboxSubmissions)?state.inboxSubmissions:[]).forEach(x=>push(x,'state-submissions')); }catch(_){ }
    try{
      const raw = localStorage.getItem(typeof LS_KEY !== 'undefined' ? LS_KEY : 'doggystyle_workspace_state_v1');
      const data = raw ? JSON.parse(raw) : {};
      (Array.isArray(data?.inboxAssignments)?data.inboxAssignments:[]).forEach(x=>push(x,'local-inboxAssignments'));
    }catch(_){ }
    return dedupe(rows).filter(r=>{
      const ps = String(r.proposalStatus || '').toLowerCase();
      return !ps || ps==='pending' || ps==='submitted';
    });
  }
  function renderRows(rows){
    ...
    listEl.innerHTML = rows.map(r=>{
      ...
      const tpl = normTemplate(r);
      const ts = Number(r.submittedAt || r.createdAt || Date.now());
      const when = isFinite(ts) ? new Date(ts).toLocaleString('de-DE') : '—';
      return `<div class="item"><strong>${escapeHtml(c.name || 'Kunde')}</strong><div class="muted">${escapeHtml(c.email || '')} · ${escapeHtml(tpl.name)} · ${escapeHtml(when)} · ${escapeHtml(String(r.__source||''))}</div></div>`;
    }).join('');
  }
  function mapIntoSelections(rows){
    const selCustomer = document.getElementById('assignCustomer');
    const selTemplate = document.getElementById('assignTemplate');
    if(!selCustomer || !selTemplate) return {customers:0, templates:0};
    const customers=[]; const templates=[]; const cseen=new Set(); const tseen=new Set();
    rows.forEach(r=>{
      const c = rowToCustomer(r); const tpl = normTemplate(r);
      const ckey = String(c.customerId || c.email || c.name || '').trim();
      if(ckey && !cseen.has(ckey)){
        cseen.add(ckey); customers.push(c);
        const label = c.email ? `${c.name} · ${c.email}` : c.name;
        addOption(selCustomer, ckey, label);
      }
      if(tpl.id && !tseen.has(tpl.id)){
        tseen.add(tpl.id); templates.push(tpl);
        addOption(selTemplate, tpl.id, tpl.name);
      }
    });
    try{ window.__dsInboxInlineCustomers = customers; }catch(_){ }
    if(customers.length && !String(selCustomer.value||'').trim()) selCustomer.value = String(customers[0].customerId || customers[0].email || customers[0].name || '');
    if(templates.length && !String(selTemplate.value||'').trim()) selTemplate.value = String(templates[0].id || '');
    return {customers:customers.length, templates:templates.length};
  }
  async function run(source){
    setText('assignMsg', 'Kundenvorschläge werden geladen …', false);
    setText('inboxProposalMsg', 'Kundenvorschläge werden geladen …', false);
    const rows = await loadRows();
    const info = mapIntoSelections(rows);
    renderRows(rows);
    const msg = rows.length ? `Kundenvorschläge geladen: ${rows.length}` : 'Keine Kundenvorschläge gefunden.';
    setText('assignMsg', msg, !rows.length);
    setText('inboxProposalMsg', msg + (source ? ` · ${source}` : ''), !rows.length);
    try{ const diag = document.getElementById('assignDiag'); if(diag) diag.textContent = `Diagnose: phase=gb23-map · proposals=${rows.length} · customers=${info.customers} · templates=${info.templates}`; }catch(_){ }
    return rows;
  }
  window.__dsGb23MapInboxProposals = run;
  const kick = ()=>{ Promise.resolve().then(()=>run('auto')).catch(err=>{ setText('assignMsg', 'Vorschläge laden fehlgeschlagen: ' + ((err&&err.message)||err||'unbekannt'), true); setText('inboxProposalMsg', 'Vorschläge laden fehlgeschlagen: ' + ((err&&err.message)||err||'unbekannt'), true); }); };
  window.addEventListener('pageshow', ()=>setTimeout(kick, 250));
  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) setTimeout(kick, 200); });
  document.querySelector('[data-tab="inbox"]')?.addEventListener('click', ()=>setTimeout(kick, 120));
  document.addEventListener('click', (ev)=>{
    const btn = ev.target && ev.target.closest ? ev.target.closest('#btnInboxRefresh, #btnInboxProposalLoad') : null;
    if(!btn) return;
...
(function(){
  ...
  const normalizeRow = (row, source)=>{
    ...
    try{
      if(typeof ensureStateShape === 'function') ensureStateShape();
      resolvedCustomer = (typeof dsFindExistingCustomerForInboxRow === 'function') ? (dsFindExistingCustomerForInboxRow(row, customer) || null) : null;
      resolvedPet = (typeof dsFindExistingPetForInboxRow === 'function') ? (dsFindExistingPetForInboxRow(resolvedCustomer || {}, pet, row) || null) : null;
      const customers = asArray(state && state.customers);
      const pets = asArray(state && (((state.pets||[]).length ? state.pets : state.dogs) || []));
      if(!resolvedCustomer && customers.length === 1) resolvedCustomer = customers[0] || null;
      if(!resolvedPet && pets.length === 1) resolvedPet = pets[0] || null;
      if(!resolvedCustomer && resolvedPet && typeof getCustomer === 'function'){
        resolvedCustomer = getCustomer(resolvedPet.customerId || resolvedPet.ownerId || '') || null;
      }
      if(resolvedCustomer && !resolvedPet){
        let own = pets.filter(function(x){
          return norm((x && (x.customerId || x.ownerId || '')) || '') === norm((resolvedCustomer && (resolvedCustomer.id || resolvedCustomer.customerId || '')) || '');
        });
        if(own.length === 1) resolvedPet = own[0] || null;
    ...
    const targetCustomerId = norm((resolvedCustomer && (resolvedCustomer.id || resolvedCustomer.customerId)) || row.customerId || customer.customerId || customer.id || '');
    const targetPetId = norm((resolvedPet && (resolvedPet.id || resolvedPet.petId)) || row.petId || pet.id || pet.petId || '');
    const sourceKey = lower((row.__sourceKey || row.__rowId || row.id || row.taskId || row.proposalId || row.submissionId) || [customerId, targetPetId || norm(pet.name || ''), submittedAt].join('|'));
    return {
      ...row,
      ...
      __template: template,
      __targetCustomerId: targetCustomerId || '',
      __targetPetId: targetPetId || '',
      __sourceKey: sourceKey || '',
      templateId: template.id,
      title,
      customerId,
      petId: norm(row.petId || pet.id || pet.petId || targetPetId || ''),
      customerUid,
      customerEmail,
      customerName,
      status,
...
function dsLaunchProposalInCustomerEditor(row){
  ...
  let ok = false;
  let lastErr = '';
  try{ ok = !!dsOpenProposalInCustomerEditor(row, { silent:false, forceMainEditor:true }); }catch(err){ lastErr = String((err && err.message) || err || 'review-open-failed'); }
  if(!ok){
