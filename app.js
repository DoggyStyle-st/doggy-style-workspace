...
function updateSyncUI(){
  ...
  // iOS/Safari kann navigator.onLine falsch melden (PWA/Cache). Wir werten daher auch Cloud-Aktivität als "online".
  const cloudActivityAt = Math.max(Number(SYNC.cloudLastSeenAt||0), Number(SYNC.cloudLastOkAt||0), Number(SYNC.cloudLastPullOkAt||0));
  const effectiveOnline = netOnline || (cloudActivityAt && (Date.now() - cloudActivityAt) < 5*60*1000);

  try{ if(pill){ pill.classList.toggle('is-online', !!effectiveOnline);
      pill.classList.toggle('is-offline', !effectiveOnline); } }catch(e){}
  const localLine = `Lokal gespeichert: ${fmtDT(SYNC.localSavedAt)}`;

  // Internet-Status (nicht gleich Cloud!)
  const netLine = `Internet: ${effectiveOnline ? 'Online' : 'Offline'}${effectiveOnline && !netOnline ? ' (Cloud aktiv)' : ''}`;
  ...
  if(!cloudIsEnabled()){
    ...
    if(!CLOUD.user){
      pillText = `${effectiveOnline ? 'Online' : 'Offline'} · Cloud: Login nötig`;
      cloudLine = 'Cloud: nicht angemeldet';
    } else if(SYNC.cloudLastError){
      pillText = `${effectiveOnline ? 'Online' : 'Offline'} · Cloud: Fehler`;
      cloudLine = `Cloud Fehler: ${SYNC.cloudLastError}`;
    } else if(SYNC.cloudPending){
      pillText = `${effectiveOnline ? 'Online' : 'Offline'} · Cloud: Sync…`;
      cloudLine = `Cloud Sync: läuft (letztes OK ${fmtDT(SYNC.cloudLastOkAt)})`;
    } else {
      pillText = `${effectiveOnline ? 'Online' : 'Offline'} · Cloud: OK`;
  ...
  if(dot){ dot.classList.toggle('online', !!effectiveOnline); dot.classList.toggle('offline', !effectiveOnline); }
...
async function cloudLoadStateWithRetry(maxTries=3){
  let lastErr = null;
  // Beim Start sofort Sync-Status anzeigen
  SYNC.cloudPending = true;
  updateSyncUI();
  for(let i=0;i<maxTries;i++){
    try{
      const remote = await cloudLoadState();
      // Read erfolgreich (auch wenn noch kein Workspace existiert)
      SYNC.cloudLastPullOkAt = Date.now();
      updateSyncUI();
      if(remote){ SYNC.cloudPending = false; SYNC.cloudLastError = ''; updateSyncUI(); return {remote, err:null}; }
    }catch(e){
      lastErr = e;
    }
  ...
  if(lastErr){ SYNC.cloudLastError = String(lastErr.message||lastErr.code||lastErr); }
  updateSyncUI();
...
// Auto-Online UX: Status sofort aktualisieren (ohne Save)
try{
