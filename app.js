...
function renderContractPanel(){
  ...
  // signature pad (robust bindings)
  try{ initContractSignaturePad(true); }catch(_){ try{ initContractSignaturePad(); }catch(__){} }

  const _cBanner = document.getElementById('contractStatusBanner');
  const _flash = (msg)=>{ if(!_cBanner) return; _cBanner.textContent = msg; setTimeout(()=>{ if(_cBanner.textContent===msg) _cBanner.textContent=''; }, 1200); };

  const btnClear = document.getElementById('contractSigClear');
  if(btnClear){
    const h = (e)=>{ if(e){ e.preventDefault(); e.stopPropagation(); } clearContractSig(); _flash('✍️ Unterschrift gelöscht'); };
    btnClear.onclick = h;
    btnClear.addEventListener('touchend', h, {passive:false});
  }

  const pdfBtn = document.getElementById('contractPdfBtn');
  if(pdfBtn){
    const h = (e)=>{ if(e){ e.preventDefault(); e.stopPropagation(); }
    ...
    pdfBtn.onclick = h;
    pdfBtn.addEventListener('touchend', h, {passive:false});
  }
  ...
  if(btnSign){
    const h = (e)=>{ if(e){ e.preventDefault(); e.stopPropagation(); }
      ...
      chk.checked = false;
      updateSignedInfo();
      _flash('✅ Vertrag unterschrieben');
      try{ renderDogs(); }catch(_){ }
    };
    btnSign.onclick = h;
    btnSign.addEventListener('touchend', h, {passive:false});
  }
