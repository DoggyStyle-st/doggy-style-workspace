...
// === ANA034_CLEAN: Super-visible Debug HUD (fixed / iPad-safe) ===
(function(){
  try {
    ...
    function ensureHud(){
      ...
      hud.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
          <div style="font-weight:800;">ANA032 DEBUG</div>
...
/* =========================
   ANA022 – Phase B (Variante 2): Auslastung im Dashboard + Belegung
   Safe-Addon: greift NICHT in Navigation/Buttons ein (nur Auswertungen)
   ========================= */
(function(){
  try{
    const BUILD_MARK = "V10FIX6-A-ANA034_CLEAN";


// === ANA034_CLEAN helpers ===
...
function ensureDbgPanel() {
  ...
  document.getElementById("dbgPanelClear")?.addEventListener("click", (e)=>{
    ...
    __DBG.buffer = [];
    dbg("ANA034_CLEAN cleared");
  }, {passive:false});
  dbg("ANA034_CLEAN panel ready");
}
...
// --- ANA033: Ensure debug panel appears even if no DOM mutations happen ---
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  dbg("ANA034_CLEAN observer active");
...
// === /ANA034_CLEAN helpers ===
    ...
    window.renderAnalyticsPanel = function(){
      ...
      try{ run(); }catch(e){ console.warn("ANA022 analytics failed", e); }
    };
    ...
    console.warn("ANA022 addon failed", e);
  }
})();

// =========================
// PHASE_B_ANALYTICS_V2 (ANA023) – Auto-Dates + Render Hooks + Debug Count
// =========================
(function(){
  try{
...
// =========================
// PHASE_B_ANALYTICS_V3 (ANA024) – Event Delegation (Fix for dynamic panels)
// =========================
(function(){
  try{
    function pad2(n){ return String(n).padStart(2,"0"); }
...
/* ===============================
   ANA034_CLEAN – Analytics Fix + Debug
   =============================== */
(() => {
  'use strict';
  const BUILD = 'ANA034_CLEAN';
...
// === ANA034_CLEAN: bind anaApply (Haupt-Aktualisieren) ===
(function(){
  try {
    const btn = document.getElementById('anaApply');
    ...
    btn.addEventListener('click', async ()=>{
      try {
        ...
        alert('ANA034_CLEAN: Fehler beim Aktualisieren: ' + (err && err.message ? err.message : String(err)));
      }
    });
...
// === ANA034_CLEAN: wrap createStay to surface errors ===
(function(){
  try {
    ...
    const wrapped = async function(...args){
      ...
      try {
        ...
        alert('ANA034_CLEAN: createStay Fehler: ' + (err && err.message ? err.message : String(err)));
        throw err;
      }
    };
