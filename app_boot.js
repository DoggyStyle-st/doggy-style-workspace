  ...
  setTimeout(() => {
    try {
      ...
      bind('btnStaySave', () => {
        ...
        try{ warnMissingSignatureIfStay(currentDoc); }catch(_){ }
    saveCurrent(true);
      });
    ...
    saveCurrent(true);
      ...
      stayRoot.querySelectorAll('button').forEach(b=>{
        ...
        if (label === 'speichern') {
          b.dataset.bound='1';
          b.addEventListener('click', (ev)=>{ ev.preventDefault(); ev.stopPropagation(); try{ syncStayEditorInputsToDoc(doc);}catch(_){} try{ warnMissingSignatureIfStay(currentDoc); }catch(_){ }
    ...
    try {
      ...
      if (btn && !btn.dataset.bound) {
        ...
        btn.addEventListener('click', (ev) => {
          ...
          try { try{ warnMissingSignatureIfStay(currentDoc); }catch(_){ }
    saveCurrent(true); } catch (e) {
...
// ===== 4B-3 Soft-Warnung: Unterschrift fehlt (nicht blockierend) =====
function warnMissingSignatureIfStay(doc){
  try{
    if(!doc) return;
    const isStay = (doc.templateName==='stay' || doc.type==='stay' || doc.templateId==='stay' || String(doc.templateId||'').startsWith('hundeannahme'));
    if(!isStay) return;

    const hasSig = !!(doc.signature && doc.signature.dataUrl);
    if(hasSig) return;

    // UI-Hinweis im Signaturbereich (falls vorhanden)
    try{
      const statusEl = document.getElementById('staySigStatus');
      if(statusEl){
    ...
    // Soft-Toast/Alert (einmalig pro Save-Klick)
    try{
      // prefer non-blocking toast if exists
      if(typeof toast === 'function') toast('⚠ Unterschrift fehlt – bitte später nachholen.');
      else if(typeof showToast === 'function') showToast('⚠ Unterschrift fehlt – bitte später nachholen.');
